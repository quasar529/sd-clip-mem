...
import math
def my_simple_scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None, args=None):
    ...
    if attn_mask is not None:
        if attn_mask.dtype == torch.bool:
            attn_bias.masked_fill_(attn_mask[0, 0].logical_not(), float("-inf"))
        else:
            attn_bias += attn_mask
    ...
    attn_weight[1::2, :, :, 0] = attn_weight[1::2, :, :, 0] * args.c1

    attn_weight = torch.softmax(attn_weight, dim=-1)

    return attn_weight @ value, attn_weight

def my_simple_scaled_dot_product_attention_inf_PT(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None, args=None):
...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        if encoder_hidden_states is not None and direct_args is not None:
            # import pdb ; pdb.set_trace()
            if direct_args.cross_attn_mask:
                attention_mask = torch.zeros(77, device='cuda').view(1, 1, 1, -1)
                if direct_args.prompt_length >= 2:
                    if direct_args.mask_length_minis1:
                        mask_length = direct_args.prompt_length - 1
                    else:
                        mask_length = direct_args.prompt_length

                attention_mask[:, :, :, :mask_length] = 1
                attention_mask = attention_mask.repeat(2, 8, hidden_states.shape[1], 1)
        ...
        if miti_mem:
            # import pdb ; pdb.set_trace()
            hidden_states, attn_weight = my_simple_scaled_dot_product_attention(
                    query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False, args=direct_args,
                )
        # elif inf_PT:
        #     hidden_states, attn_weight = my_simple_scaled_dot_product_attention_inf_PT(
        #             query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False, args=direct_args,
        #         )
        else:
            hidden_states = F.scaled_dot_product_attention(
        ...
        if return_attention and not miti_mem:
            import math
            # weights = torch.matmul(query, key.transpose(2, 3)) / torch.sqrt(head_dim)
            L, S = query.size(-2), key.size(-2)
            # scale_factor = 1 / math.sqrt(query.size(-1)) if scale is None else scale
            ...
            if attention_mask is not None:
                if attention_mask.dtype == torch.bool:
                    attn_bias.masked_fill_(attention_mask[0, 0].logical_not(), float("-inf"))
                else:
        ...
        if return_attention:
            #breakpoint()
            return hidden_states, None# attn_weight.detach().cpu().type(torch.float16).numpy()
        else:
            return hidden_states
