...
def get_dataset_finetune(dataset_name, non_mem_dataset=None, end=None, repeats=1, non_mem_ratio=0):
...
def load_and_process_images(images, transform, device, num_workers=16):
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        futures = {
            executor.submit(load_and_transform, image, transform, device): idx for idx, image in enumerate(images)
        }
        transformed_images = [None] * len(images)
...
def get_SSCD_feature(images, model, device):
    with torch.no_grad():
        ...
        images = process_images(images, skew_320, device, num_workers=16)
